...
client.on('connect', () => {
  console.log('Connected to MQTT broker');
  client.subscribe('gas_sensor/readings', (err) => {
    if (!err) {
      console.log('Subscribed to gas_sensor/readings topic');
    } else {
      console.error('Failed to subscribe:', err);
    }
  });
...
client.on('message', (topic, message) => {
  if (topic === 'gas_sensor/readings') {
    updateGasValue(parseFloat(message.toString()));
  }
});
client.on('message', (topic, message) => {
...
/***************************************************************************/
    // Create the gas sensor gauge
    var gasGauge = new Gauge(document.getElementById("gas")).setOptions({
      ...
      limitMax: false,     // If false, the max value of the gauge is 100 by default
      limitMin: false,     // If false, the min value of the gauge is 0 by default
      colorStart: '#6FADCF',   // Colors for the gauge range
      colorStop: '#8FC0DA',    
      strokeColor: '#FFFF50',  // Gauge stroke
      generateGradient: true
  });
  
  gasGauge.maxValue = 1000; // Maximum value for gas sensor
  gasGauge.setMinValue(0); // Minimum value
  gasGauge.animationSpeed = 32; // How fast the needle moves
  
  // Initialize gauge with 0 value
  gasGauge.set(0);
  
  // Function to update gas sensor value (retrieved from your MQTT or other backend)
  function updateGasValue(gasValue) {
      // Update gauge and displayed value
      gasGauge.set(gasValue);
      document.getElementById("gas-val").innerText = gasValue;
  }
  
  // Example: Simulate receiving data from the gas sensor every 5 seconds
  // Replace this with your actual sensor data logic
  // setInterval(function() {
  //     // Simulate getting a random gas sensor value (replace with actual sensor data)
  //     var gasSensorValue = Math.floor(Math.random() * 101); // Random value between 0 and 100
  //     updateGasValue(gasSensorValue);
